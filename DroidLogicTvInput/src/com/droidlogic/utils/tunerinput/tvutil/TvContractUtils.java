...
public class TvContractUtils {
	...
	public static void updateChannelsDTV(
		Context context, String inputId, List<ChannelInfo> channels) {
		// Create a map from service ID to channel row ID for existing channels.
		...
		try {
			...
			mExistingChannelsMap.put(serviceId, rowId);
			}
		} finally {
			if (cursor != null) {
				cursor.close();
		...
		for (ChannelInfo channel : channels) {
			...
			Long rowId = mExistingChannelsMap.get(channel.serviceId);
			Uri uri;
			if (rowId == null) {
				Log.d(TAG, "New service:"+channel.serviceId);
				uri = resolver.insert(TvContract.Channels.CONTENT_URI, values);
			} else {
			Log.d(TAG, "Update service:"+channel.serviceId);
		...
		Log.d(TAG, "Deletes "+size+" services");
	}

	public static void updateChannelsATV(
		Context context, String inputId, List<ChannelInfo> channels) {
			// Create a map from service ID to channel row ID for existing channels.
			SparseArray<Long> mExistingChannelsMap = new SparseArray<Long>();
			Uri channelsUri = TvContract.buildChannelsUriForInput(inputId);
			...
			try {
				...
				while (cursor != null && cursor.moveToNext()) {
					...
					mExistingChannelsMap.put(frequency, rowId);
				}
			} finally {
				if (cursor != null) {
					cursor.close();
			...
			for (ChannelInfo channel : channels) {
				...
				String output = MapUtil.mapToString(map);
				values.put(TvContract.Channels.COLUMN_INTERNAL_PROVIDER_DATA, output);
				Long rowId = mExistingChannelsMap.get(channel.frequency);
				Uri uri;
				if (rowId == null) {
					Log.d(TAG, "New service:"+channel.frequency);
					uri = resolver.insert(TvContract.Channels.CONTENT_URI, values);
				} else {
					Log.d(TAG, "Update service:"+channel.frequency);
					uri = TvContract.buildChannelUri(rowId);
					resolver.update(uri, values, null, null);
					mExistingChannelsMap.remove(channel.frequency);
				}
				if (!TextUtils.isEmpty(channel.logoUrl)) {
					logos.put(TvContract.buildChannelLogoUri(uri), channel.logoUrl);
				}
	...
	public static ChannelInfo getChannelInfoATV(ContentResolver resolver, Uri channelUri) {
		...
		try {
			...
			cursor.moveToNext();
			{
				...
				info = new ChannelInfo(number, name, null, 0, 0, 0, 0, 0,
							0,
							Integer.parseInt(parsedMap.get("stype")),
							Integer.parseInt(parsedMap.get("freq")),
							0 ,//bandwidth
							0,// videoPID
							0,//videoFormat,
							null,// audioPIDs[],
							null,// audioFormats[],
							0,// pcrPID,
							Integer.parseInt(parsedMap.get("vstd")),
							Integer.parseInt(parsedMap.get("astd")),
							Integer.parseInt(parsedMap.get("auto")),
